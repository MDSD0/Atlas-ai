...
const ATLAS_MARKER: &[u8] = b"notify;Atlas;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(event) = pt.strip_prefix(ATLAS_MARKER) {
...
mod tests {
    ...
    #[test]
    fn atlas_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;Atlas;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;Atlas;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;Atlas;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;Atlas;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn atlas_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Atlas;attention")),
