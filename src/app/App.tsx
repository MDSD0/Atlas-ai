...
const SIDEBAR_WIDTH_STORAGE_KEY = "atlas.sidebar.width";
const SIDEBAR_VIEW_STORAGE_KEY = "atlas.sidebar.view";
const AGENT_PANEL_DEFAULT_WIDTH = 420;
const AGENT_PANEL_MIN_WIDTH = 320;
const AGENT_PANEL_MAX_WIDTH = 620;
const AGENT_PANEL_WIDTH_STORAGE_KEY = "atlas.ai.rightPanel.width";
...
export default function App() {
  ...
  const agentPanelWidthRef = useRef(readAgentPanelWidth());
  const agentPanelWidthWriteTimerRef = useRef(0);
  ...
  const persistAgentPanelWidth = useCallback((next: number) => {
    const clamped = clampAgentPanelWidth(next);
    agentPanelWidthRef.current = clamped;
    if (agentPanelWidthWriteTimerRef.current) {
      window.clearTimeout(agentPanelWidthWriteTimerRef.current);
    }
    agentPanelWidthWriteTimerRef.current = window.setTimeout(() => {
      agentPanelWidthWriteTimerRef.current = 0;
      try {
  ...
  const switchWorkspace = useCallback(
    ...
    [workspaceEnv, setWorkspaceEnv, resetToAgentHome],
  );
  useEffect(() => {
    // Load recent workspaces from persisted settings on mount
    void useWorkspaceStore.getState().loadRecents();
  }, []);

  const [shortcutsOpen, setShortcutsOpen] = useState(false);
  const [newEditorOpen, setNewEditorOpen] = useState(false);
  const activeSessionId = useChatStore((s) => s.activeSessionId);
  const executionCwdMode = useChatStore((s) => s.executionCwdMode);
  const openMini = useChatStore((s) => s.openMini);
  const focusInput = useChatStore((s) => s.focusInput);
  const miniOpen = useChatStore((s) => s.mini.open);
  const miniDock = useChatStore((s) => s.mini.dock);
  ...
  const agentDockOpen = miniOpen && miniDock === "right" && !isWelcomeTab;
      ...
      (event) => {
        ...
        void (async () => {
          try {
            ...
            if (!parsed.ok) {
              console.warn("[atlas] theme not applied:", parsed.error);
              return;
            }
            await saveCustomTheme(parsed.theme);
          } catch (e) {
            console.warn("[atlas] theme ingest failed:", e);
          }
        })();
      },
    );
  ...
  // activeFolder tracks the last folder the user explicitly navigated to in
  // the explorer. New terminals start here. It is reset when workspace changes.
  ...
  // Read workspaceRoot directly for contexts that need it without going
  // through the hook (setLive, sourceControl fallback, etc.)
  const workspaceRoot = useWorkspaceStore((s) => s.workspaceRoot);

  // Reset activeFolder when the workspace changes so new terminals
  // don't inherit a stale subfolder from the previous workspace.
  useEffect(() => {
  ...
  }, [hasComposer, isWelcomeTab, miniOpen, openMini, focusInput]);

  const openAgentHome = useCallback(() => {
    useChatStore.getState().closeMini();
    goToWelcome();
    focusInput(null);
  }, [goToWelcome, focusInput]);
  ...
  const setAgentActiveFolder = useCallback(
    (path: string) => {
      const root = useWorkspaceStore.getState().workspaceRoot;
      if (!root) return;
      ...
      if (sidebarView !== "explorer") persistSidebarView("explorer");
      requestAnimationFrame(() => explorerRef.current?.revealPath(normalizedPath));
    },
    [persistSidebarView, sidebarView],
  );
  ...
  const projectName = (() => {
    if (!workspaceRoot) return "Unbound";
    const parts = workspaceRoot.replace(/\\/g, "/").split("/").filter(Boolean);
    return parts.length ? parts[parts.length - 1] : workspaceRoot;
  })();

  const executionCwd = (() => {
    if (executionCwdMode === "activeTerminal") {
      return activeTerminalLeafCwd ?? activeFolder ?? workspaceRoot;
    }
    if (executionCwdMode === "activeFolder") {
      return activeFolder ?? workspaceRoot;
    }
  ...
  const sourceControlContextPath = (() => {
    if (activeTab?.kind === "terminal") {
      return activeTerminalLeafCwd ?? explorerRoot ?? workspaceRoot;
    }
    ...
    return explorerRoot ?? workspaceRoot;
  })();
  ...
  const badgeContextPath = workspaceRoot;
  ...
  const handleTerminalCwd = useCallback(
    (leafId: number, cwd: string) => {
      ...
      // Only authorize and propagate if cwd is within workspaceRoot boundary.
      // Never let a terminal cd outside the workspace update the explorer root.
      const root = useWorkspaceStore.getState().workspaceRoot;
      const inBoundary =
        !root || cwd === root || cwd.startsWith(root + "/") || cwd.startsWith(root + "\\");
      if (inBoundary && cwd && !authorizedCwds.current.has(cwd)) {
  ...
  const statusPathMode = isWelcomeTab
    ? "agent"
    : activeFilePath
      ? "file"
      : "terminal";
  const statusPath = isWelcomeTab
    ? activeFolder ?? workspaceRoot
    : activeCwd;
  const statusPathHandler = isWelcomeTab ? setAgentActiveFolder : sendCd;

  const restoreProjectWorkspace = useCallback(
    async (nextRoot: string | null): Promise<boolean> => {
      const currentRoot = useWorkspaceStore.getState().workspaceRoot;
  ...
  const workspaceSurface = (
    <div className="relative h-full min-h-0">
      {/* Welcome screen — shown when no other tab type is active */}
      {isWelcomeTab && (
    ...
    <ThemeProvider>
      <TooltipProvider>
        <div className="relative flex h-screen flex-col overflow-hidden bg-background text-foreground">
            ...
            >
              ...
              {agentDockOpen ? (
                <>
                  <ResizableHandle
                    withHandle
                    className="bg-border/45 backdrop-blur-xl backdrop-saturate-150 [&>div]:bg-border/70"
                  />
                  <ResizablePanel
                    id="agent-panel"
                    defaultSize={`${agentPanelWidthRef.current}px`}
                    minSize={`${AGENT_PANEL_MIN_WIDTH}px`}
                    maxSize={`${AGENT_PANEL_MAX_WIDTH}px`}
                    className="bg-card/60 backdrop-blur-2xl backdrop-saturate-150"
                    onResize={(size) => {
                      if (size.inPixels > 0) {
                        persistAgentPanelWidth(size.inPixels);
                      }
                    }}
                  >
                    <AiDockedPanel />
          ...
          <StatusBar
            cwd={statusPath}
            filePath={isWelcomeTab ? null : activeFilePath}
            home={home}
            pathMode={statusPathMode}
            workspaceRoot={workspaceRoot}
            onCd={statusPathHandler}
          ...
          <AnimatePresence>
            {miniOpen && miniDock === "floating" && !isWelcomeTab ? (
