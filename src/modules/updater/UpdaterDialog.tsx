...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S atlas-bin";
    case "debian":
      return `sudo apt install ./Atlas_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Atlas-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Atlas v${manual.version} is available`
                  : `Atlas v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Atlas to finish installing."
              : downloading
                ? progress !== null
