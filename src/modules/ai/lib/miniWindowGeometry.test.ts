...
describe("docked geometry", () => {
  it("keeps the right edge anchored", () => {
    const g = defaultDockedGeom(vp);
    expect(g.x + g.w).toBe(vp.vw - 16);
  });

  it("reanchors after clamping", () => {
    const g = clampDockedGeom({ x: 10, y: 20, w: 500, h: 400 }, vp);
    expect(g.x + g.w).toBe(vp.vw - 16);
  });

  it("resizes while staying attached to the right side", () => {
    const start: Geom = { x: 924, y: 16, w: 500, h: 800 };
    const g = applyDockedResize(start, "w", -80, 0, vp);
    expect(g.w).toBe(580);
    expect(g.x + g.w).toBe(vp.vw - 16);
  });
});
