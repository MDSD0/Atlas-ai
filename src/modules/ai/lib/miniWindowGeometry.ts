...
export type MiniWindowDock = "floating" | "right";
...
export function defaultGeom(vp: Viewport): Geom {
  ...
  return clampGeom(
    { x: vp.vw - w - MARGIN_X, y: TOP_GAP, w, h },
    vp,
  );
}

export function defaultDockedGeom(vp: Viewport): Geom {
  const w = Math.max(MIN_W, Math.min(460, vp.vw - MARGIN_X * 2));
  const h = Math.max(MIN_H, vp.vh - TOP_GAP * 2);
  return clampDockedGeom({ x: vp.vw - w - MARGIN_X, y: TOP_GAP, w, h }, vp);
}
...
export function clampDockedGeom(g: Geom, vp: Viewport): Geom {
  const clamped = clampGeom(g, vp);
  return {
    ...clamped,
    x: Math.max(0, vp.vw - clamped.w - MARGIN_X),
  };
}
...
export function applyDockedResize(
  start: Geom,
  dir: ResizeDir,
  dx: number,
  dy: number,
  ...
  return clampDockedGeom(applyResize(start, dir, dx, dy, vp), vp);
}
