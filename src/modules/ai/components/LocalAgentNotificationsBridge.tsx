...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Atlas needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Atlas run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Atlas finished", "Your task is ready");
    }
  }, [status, error]);
