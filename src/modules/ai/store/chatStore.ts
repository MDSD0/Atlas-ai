...
type Live = {
  ...
  restoreProjectWorkspace?: (workspaceRoot: string | null) => Promise<boolean>;
};
...
type StoreState = {
  ...
  closeMini: () => void;
  toggleMini: () => void;
  setMiniDock: (dock: MiniState["dock"]) => void;
...
const NOOP_LIVE: Live = {
  ...
  readLeafBuffer: () => null,
  restoreProjectWorkspace: async (workspaceRoot: string | null) => {
    const store = useWorkspaceStore.getState();
    if (workspaceRoot) await store.setWorkspaceRoot(workspaceRoot);
...
function readMiniDock(): MiniState["dock"] {
  try {
    const stored = window.localStorage.getItem(MINI_DOCK_STORAGE_KEY);
    return stored === "right" ? "right" : "floating";
  } catch {
    return "floating";
  }
}

function saveMiniDock(dock: MiniState["dock"]) {
  try {
    window.localStorage.setItem(MINI_DOCK_STORAGE_KEY, dock);
  } catch {
...
async function restoreWorkspaceForSession(meta: SessionMeta): Promise<boolean> {
  const workspaceRoot = meta.workspaceRoot ?? meta.projectId ?? null;
  const live = useChatStore.getState().live;
  if (live.restoreProjectWorkspace) {
    return live.restoreProjectWorkspace(workspaceRoot);
  }
  const store = useWorkspaceStore.getState();
  if (workspaceRoot) await store.setWorkspaceRoot(workspaceRoot);
  else store.clearWorkspace();
...
function makeChat(sessionId: string): Chat<UIMessage> {
  ...
  const toolContext: ToolContext = {
    ...
    getProjectContext: () =>
      useChatStore.getState().live.getProjectContext(),
    getTerminalContext: () =>
...
export const useChatStore = create<StoreState>((set, get) => ({
  ...
  closeMini: () => set((s) => ({ mini: { ...s.mini, open: false } })),
  toggleMini: () =>
    set((s) => ({ mini: { ...s.mini, open: !s.mini.open } })),
  setMiniDock: (dock) => {
    saveMiniDock(dock);
    set((s) => ({ mini: { ...s.mini, dock } }));
  },
  ...
  hydrateSessions: async () => {
    ...
    let freshId: string;
    if (reusable) {
      nextSessions = normalized;
      freshId = reusable.id;
    } else {
      freshId = newSessionId();
      const fresh = bindSessionToWorkspace(
        {
          id: freshId,
          title: "New chat",
          createdAt: Date.now(),
  ...
  newSession: () => {
    ...
    const meta: SessionMeta = bindSessionToWorkspace(
      {
        id,
        title: "New chat",
        createdAt: Date.now(),
