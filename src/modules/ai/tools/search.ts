...
function resolveRoot(
  ...
  if (rawRoot && rawRoot.trim().length > 0) {
    try {
      return {
        ok: true,
        path: resolveSearchRoot(rawRoot, ctx.getProjectContext()),
      };
    } catch (e) {
  ...
  try {
    return {
      ok: true,
      path: resolveSearchRoot(undefined, ctx.getProjectContext()),
    };
  } catch (e) {
    return { ok: false, error: String(e) };
  }
    ...
    grep: tool({
      ...
      inputSchema: z.object({
        ...
        root: z
          ...
          .optional()
          .describe(
            "Root to search under. Defaults to workspace root.",
          ),
        glob: z
          .array(z.string())
          .optional()
