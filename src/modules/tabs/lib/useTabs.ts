...
export function useTabs() {
  ...
  const resetToAgentHome = useCallback(() => {
    const tabId = nextIdRef.current++;
    let agentId = tabId;
    let toDispose: number[] = [];
    setTabs((curr) => {
      ...
      const existing = curr.find((t) => t.kind === "welcome");
      if (existing) {
        agentId = existing.id;
        return [{ ...existing, title: "Agent" }];
      }
      return [{ id: tabId, kind: "welcome", title: "Agent" }];
    });
    setActiveId(agentId);
    for (const lid of toDispose) disposeSession(lid);
  }, []);
  ...
  const goToWelcome = useCallback(() => {
    const existing = tabsRef.current.find((t) => t.kind === "welcome");
    if (existing) {
      setActiveId(existing.id);
      return;
    }
    const tabId = nextIdRef.current++;
    setTabs((t) => [{ id: tabId, kind: "welcome", title: "Agent" }, ...t]);
    setActiveId(tabId);
  }, []);
