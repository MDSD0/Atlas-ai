...
type Props = {
  ...
  /**
   * Called when the user explicitly navigates into a folder.
   * App.tsx stores this as activeFolder so new terminals open there.
   * The explorer itself does not change its root -- that is always workspaceRoot.
   */
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        revealPath: (path: string) => {
          if (!rootPath) return;
          // Expand every ancestor between rootPath and path
          const parts: string[] = [];
          let cur = path;
          while (cur !== rootPath && cur.length > rootPath.length) {
            const parent = cur.slice(0, cur.lastIndexOf("/"));
            if (!parent || parent === cur) break;
            parts.unshift(parent);
            cur = parent;
          }
          for (const p of parts) tree.expand(p);
          setSelectedPath(path);
          requestAnimationFrame(() => scrollEntryIntoView(path));
        },
      }),
      [entryPaths, rootPath, scrollEntryIntoView, selectedPath, tree],
    );
    ...
    return (
      ...
      >
        <div className="flex h-8 shrink-0 items-center gap-1 border-b border-border/60 px-2">
          ...
          >
            <Search01Icon size={13} strokeWidth={1.5} />
          </Button>
          ...
          <Button
            ...
            title="New file"
          >
            <FileAddIcon size={13} strokeWidth={1.5} />
          </Button>
          <Button
            variant="ghost"
            size="icon"
